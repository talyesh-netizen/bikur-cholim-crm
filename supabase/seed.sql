-- Fictional demonstration data for a real Supabase project.
--
-- No real residents, families, staff, or facilities appear anywhere in
-- this file — every name below is invented for testing purposes.
--
-- HOW TO RUN THIS:
--   1. First run `npm run seed:users` (see scripts/seed-demo-users.ts),
--      which creates three fictional demo staff accounts using the
--      Supabase Admin API. This file assumes those accounts already
--      exist — it looks them up by email below.
--   2. Then run the contents of this file against the same Supabase
--      project (e.g., paste it into the Supabase SQL Editor, or run it
--      with `psql` / `supabase db push` depending on your workflow).
--
-- This is meant for a fresh project used to build/test this app, never
-- for a project already holding real resident data.
...
-- ---------------------------------------------------------------------
-- A realistic scenario: Herman Rosen is discharged from the hospital
-- directly to a rehab facility (Parkview) rather than back to
-- Cedarbrook right away. This demonstrates the facility-history
-- timeline with more than one stay.
--
-- Note: this updates current_facility_id directly rather than calling
-- the transfer_resident() function, specifically because that function
-- looks up the signed-in user via auth.uid() to attribute the transfer
-- — which isn't set when running a seed script directly against the
...
update public.residents
  set current_facility_id = (select id from public.facilities where name = 'Parkview Rehabilitation Center'),
      status = 'active'
  where first_name = 'Herman' and last_name = 'Rosen';

insert into public.interactions (interaction_type, facility_id, resident_id, staff_member_id, notes, outcome) values (
  'other',
  (select id from public.facilities where name = 'Parkview Rehabilitation Center'),
  (select id from public.residents where first_name = 'Herman' and last_name = 'Rosen'),
  (select id from auth.users where email = 'noah.fischer@example.org'),
  'Family notified of the transfer.',
  'Resident transferred to this facility. Reason: discharged from hospital; short-term rehab stay before returning to Cedarbrook.'
);
